...
Unprotect[Normalize, Bivector, BivectorSelection, DisplayForm, Format,
     GradeSelection, Scalar, ScalarProduct, ScalarSelection, ScalarValue,
     StandardForm, TeXForm, TraditionalForm, Vector, VectorSelection, e]

ClearAll[Bivector, BivectorSelection, GAdisplay, GradeSelection, Scalar,
     ScalarProduct, ScalarSelection, ScalarValue, Vector, VectorSelection,
     antisymmetric, binaryOperator, bivectorQ, bladeQ, bold, displayMapping,
     esub, multivector, oneTeXForm, gradeSelect, pmagnitude, scalarQ, signedSymmetric,
     symmetric, vectorQ]
...
Vector[v_, k_Integer /; k == 1] :=
    multivector[1, 0, v];

Vector[v_, k_Integer /; k == 2] :=
    multivector[1, 0, I v];

Bivector[v_] :=
    multivector[2, I v, 0];

gradeQ[m_multivector, n_Integer] :=
    ((m // First) == n)

scalarQ[m_multivector] :=
    gradeQ[m, 0]

vectorQ[m_multivector] :=
    gradeQ[m, 1]

bivectorQ[m_multivector] :=
    gradeQ[m, 2]

bladeQ[m_multivector] :=
    ((m // First) >= 0)

multivectorQ[m_multivector] :=
    True
...
gradeSelect[m_multivector, 0] :=
    multivector[0, (m[[2]] // Re), 0]

gradeSelect[m_multivector, 1] :=
    multivector[1, 0, m[[3]]]

gradeSelect[m_multivector, 2] :=
    multivector[2, I * (m[[2]] // Im), 0]
...
GradeSelection[m_, k_Integer /; k >= 0 && k <= 2] :=
    gradeSelect[m, k];

ScalarSelection[v_multivector] :=
    GradeSelection[v, 0];

ScalarSelection[v_multivector, True] :=
    GradeSelection[v, 0];

ScalarSelection[v_multivector, False] :=
    AngleBracket[v];

VectorSelection[v_multivector] :=
    GradeSelection[v, 1];

VectorSelection[v_multivector, True] :=
    GradeSelection[v, 1];

VectorSelection[v_multivector, False] :=
    (GradeSelection[v, 1] // Last) // ReIm;

BivectorSelection[v_multivector] :=
    GradeSelection[v, 2];

BivectorSelection[v_multivector, True] :=
    GradeSelection[v, 2];

BivectorSelection[v_multivector, False] :=
    (GradeSelection[v, 2][[2]]) // Im;

binaryOperator[f_, b_?bladeQ, m_multivector] :=
    Total[f[b, #]& /@ (GradeSelection[m, #]& /@ (Range[2 + 1] - 1))]

binaryOperator[f_, m_multivector, b_?bladeQ] :=
    Total[f[#, b]& /@ (GradeSelection[m, #]& /@ (Range[2 + 1] - 1))]

binaryOperator[f_, m1_multivector, m2_multivector] :=
    Total[f[# // First, # // Last]& /@ ({GradeSelection[m1, #]& /@ (Range[
        2 + 1] - 1), GradeSelection[m2, #]& /@ (Range[2 + 1] - 1)} // Transpose
        )]
...
multivector /: Times[multivector[n_, z1_, z2_], k_] :=
    multivector[n, k z1, k z2]

multivector /: Times[k_, multivector[n_, z1_, z2_]] :=
    multivector[n, k z1, k z2]

(* Vector inversion: Times[ Vector[], -1 ] *)

multivector /: Power[multivector[1, _, s_], -1] :=
    multivector[1, 0, s] * Power[s Conjugate[s], -1]
...
multivector /: -multivector[k_, z1_, z2_] :=
    multivector[k, -z1, -z2];

(* Scalar Times Blade *)

multivector /: (v_?notMultivectorQ) multivector[k_, z1_, z2_] :=
    multivector[k, v z1, v z2];

multivector /: multivector[0, s_, _] multivector[k_, q1_, q2_] :=
    multivector[k, s q1, s q2];
...
multivector /: multivector[1, _, s_] ** multivector[1, _, s_] :=
    multivector[0, s Conjugate[s], 0];

(* vector ** vector *)

multivector /: multivector[1, _, s_] ** multivector[1, _, t_] :=
    multivector[-1, t Conjugate[s], 0];

(* scalar or bivector or 0,2 multivector products: M N \sim m_1 n_1 *)

(* scalar ** scalar *)

multivector /: multivector[0, s_, _] ** multivector[0, t_, _] :=
    multivector[0, s t, 0];

(* bivector ** bivector *)

multivector /: multivector[2, s_, _] ** multivector[2, t_, _] :=
    multivector[0, s t, 0];
...
multivector /: multivector[k_, z1_, z2_] ** multivector[0, s_, _] :=
...
multivector /: multivector[1, _, m2_] ** multivector[2, n1_, _] :=
    multivector[1, 0, (n1 // Im) I m2]

(* M_2 N_1 \sim \lr{ 0, - m_{12} i n_2 }. *)

multivector /: multivector[2, m1_, _] ** multivector[1, _, n2_] :=
    multivector[1, 0, -(m1 // Im) I n2]
...
signedSymmetric[v1_, v2_, s_] :=
    (1/2) v1 ** v2 + (1/2) s v2 ** v1

symmetric[v1_, v2_] :=
    signedSymmetric[v1, v2, 1];

antisymmetric[v1_, v2_] :=
    signedSymmetric[v1, v2, -1];
...
multivector /: multivector[0, s_, _] . multivector[k_, z1_, z2_] :=
    multivector[k, s z1, s z2];

multivector /: multivector[k_, z1_, z2_] . multivector[0, s_, _] :=
    multivector[k, s z1, s s2];

multivector /: (v_?vectorQ) . multivector[1, _, z2_] :=
    symmetric[v, multivector[1, 0, z2]];

multivector /: (v_?vectorQ) . multivector[2, z1_, _] :=
    antisymmetric[v, multivector[2, z1, 0]];

multivector /: (b_?bivectorQ) . multivector[1, _, z2_] :=
    antisymmetric[b, multivector[1, 0, z2]];

multivector /: (b_?bivectorQ) . multivector[2, z1_, _] :=
    symmetric[b, multivector[2, z1, 0]];

multivector /: multivector[g1_, z1_, z2_] . multivector[g2_, q1_, q2_
    ] :=
    binaryOperator[Dot, multivector[g1, z1, z2], multivector[g2, q1, 
        q2]];
...
multivector /: multivector[0, s_, _] \[Wedge] multivector[k_, z1_, z2] :=
    multivector[k, s z1, s z2];

multivector /: multivector[k_, z1_, z2_] \[Wedge] multivector[0 _, s_] :=
    multivector[k, s z1, s z2];

multivector /: multivector[1, _, v1_] \[Wedge] (v2_?vectorQ) :=
    antisymmetric[multivector[1, 0, v1], v2];

(* Only e12 ^ scalar is non zero, and that is handled above *)

multivector /: multivector[2, _, _] \[Wedge] b_?bladeQ :=
    0;

multivector /: b_?bladeQ \[Wedge] multivector[2, _, _] :=
    0;

multivector /: multivector[g1_, z1_, z2_] \[Wedge] multivector[g2_, q1_, q2_
    ] :=
    binaryOperator[Wedge, multivector[g1, z1, z2], multivector[g2, q1,
         q2]];

(*AngleBracket,single operand forms,enter with[Esc]<[Esc] \
v[Esc]>[Esc]*)

multivector /: AngleBracket[multivector[1, _, _]] :=
    0

multivector /: AngleBracket[multivector[2, _, _]] :=
    0

multivector /: AngleBracket[multivector[_, z1_, _]] :=
    (z1 // Re)

ScalarValue[m_multivector] :=
    AngleBracket[m];
...
multivector /: AngleBracket[multivector[0, z1_, _], multivector[0, q1_,
     _]] :=
    Re[z1 q1]

multivector /: AngleBracket[multivector[0, z1_, _], multivector[-1, q1_,
     q2_]] :=
    Re[z1 q1]

multivector /: AngleBracket[multivector[-1, z1_, _], multivector[0, q1_,
     _]] :=
    Re[z1 q1]

multivector /: AngleBracket[multivector[0, z1_, _], multivector[_, _,
     _]] :=
    0;

multivector /: AngleBracket[multivector[_, _, _], multivector[0, q1_,
     _]] :=
    0;

multivector /: AngleBracket[multivector[k1_, z1_, z2_], multivector[k2_,
     q1_, q2_]] :=
    Re[z1 q1 + z2 Conjugate[q2]]

ScalarProduct[m1_multivector, m2_multivector] :=
    AngleBracket[m1, m2];
...
displayMapping = {{Scalar[1], 1, 1, 1}, {Vector[1, 1], esub[1], e[1],
     "\\mathbf{e}_1"}, {Vector[1, 2], esub[2], e[2], "\\mathbf{e}_2"}, {Bivector[
    -1], esub["12"], e[1] e[2], "\\mathbf{e}_{12}"}};

GAdisplay[v_grade, how_] :=
    Total[Times[#[[1]], #[[2]]]& /@ (Cases[{AngleBracket[# // First, 
        v], #[[how]]}& /@ displayMapping, {a_, b_} /; !PossibleZeroQ[a]])]

TraditionalForm[m_multivector] :=
    ((GAdisplay[m, 2]) // TraditionalForm);

DisplayForm[m_multivector] :=
    GAdisplay[m, 2];

Format[m_multivector] :=
    GAdisplay[m, 2];

StandardForm[m_multivector] :=
    GAdisplay[m, 3];
...
Protect[Normalize, TraditionalForm, DisplayForm, StandardForm, Format,
     TeXForm, Scalar, Vector, Bivector, GradeSelection, ScalarSelection, 
    VectorSelection, BivectorSelection, e, ScalarValue, ScalarProduct]
